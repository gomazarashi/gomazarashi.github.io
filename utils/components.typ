...
// Icon path data from Bootstrap Icons v1.13.1 (MIT).
#let icon-paths = (
  github: (
...
#let icon(name) = html.elem(
  ...
  ..icon-paths.at(name).map(d => html.elem("path", attrs: (d: d))),
)

// GitHub link with a decorative icon hidden from assistive technology.
#let github-link(url, label) = link(url, html.span(
  class: "github-link",
  (
    html.span(class: "github-link-icon", icon("github")),
